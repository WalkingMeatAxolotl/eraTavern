...
export default function VariableManager({ selectedAddon, onEditingChange }: { selectedAddon: string | null; onEditingChange?: (editing: boolean) => void }) {
  ...
  const { tagGrouped, untagged } = useMemo(() => {
    ...
    for (const v of filteredVars) {
      const vTags = v.tags ?? [];
      if (vTags.length === 0) { noTag.push(v); continue; }
  ...
  if (editingId !== null) {
    ...
    return (
      <VariableEditor
        ...
        isNew={isNew} allTags={allTags} allVariables={variables}
        definitions={definitions} onBack={handleBack}
      />
    );
  }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ display: "flex", gap: "2px" }}>
            <button className="vm-view-tab" onClick={() => setViewMode("byTag")} style={viewTabStyle(viewMode === "byTag")}>
              按标签
            </button>
            <button className="vm-view-tab" onClick={() => setViewMode("byVar")} style={viewTabStyle(viewMode === "byVar")}>
              按变量
            </button>
          </div>
        </div>
        <div style={{ display: "flex", gap: "6px" }}>
          {!readOnly && (
            <button className="vm-action-btn" onClick={() => setShowTagManager((v) => !v)}
              style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: "4px" }}>
            <input
              ...
              placeholder="新标签名..."
            />
            <button className="vm-action-btn" onClick={handleAddTag} style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      ...
      {visibleTags.map((tag) => {
        ...
        return (
          <div key={tag}>
            <button className="vm-cat-btn" onClick={() => onToggleCollapse(`tag:${tag}`)}
              style={{
...
function ByVarView({ filteredVars, varTagsMap, tagVarNames, onEditVar }: {
  ...
  const groups = useMemo(() => {
    ...
    for (const v of filteredVars) {
      if ((v.tags ?? []).length > 0) tagged.push(v); else untagged.push(v);
    }
...
function VarRow({ variable, tags, onEdit, onTagHover, onTagLeave }: {
