...
export default function ItemManager({ selectedAddon, onEditingChange }: { selectedAddon: string | null; onEditingChange?: (editing: boolean) => void }) {
  ...
  const handleEdit = (id: string) => { setIsNew(false); setEditingId(id); };
  const handleNew = () => { setIsNew(true); setEditingId("__new__"); };
  ...
  const handleAddTag = async () => {
    ...
    if (result.success) { setAllTags((prev) => [...prev, t]); setNewTagInput(""); }
  };

  const handleDeleteTag = async (tag: string) => {
    const result = await deleteItemTag(tag);
    if (result.success) { setAllTags((prev) => prev.filter((t) => t !== tag)); }
  };
  ...
  const { tagGrouped, untagged } = useMemo(() => {
    ...
    for (const item of filteredItems) {
      ...
      if (itemTags.length === 0) { noTag.push(item); continue; }
  ...
  if (editingId !== null) {
    ...
    const blank: ItemDefinition = {
      ...
      maxStack: 1, sellable: true, price: 0, source: selectedAddon ?? "",
    };
    return (
      <ItemEditor
        item={isNew ? blank : (existing ?? blank)}
        isNew={isNew} allTags={allTags} onBack={handleBack}
      />
    );
  }
    ...
    <div style={{ fontSize: "13px", color: T.text, padding: "12px 0" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ display: "flex", gap: "2px" }}>
            <button className="im-view-tab" onClick={() => setViewMode("byTag")} style={viewTabStyle(viewMode === "byTag")}>
              按标签
            </button>
            <button className="im-view-tab" onClick={() => setViewMode("byItem")} style={viewTabStyle(viewMode === "byItem")}>
              按物品
            </button>
          </div>
        </div>
        <div style={{ display: "flex", gap: "6px" }}>
          {!readOnly && (
            <button className="im-action-btn" onClick={() => setShowTagManager((v) => !v)}
              style={{
          ...
          {!readOnly && (
            <button className="im-action-btn" onClick={handleNew}
              style={{
                padding: "4px 12px", backgroundColor: T.bg2,
      ...
      {showTagManager && (
        <div style={{
          ...
          backgroundColor: T.bg1, border: `1px solid ${T.border}`, borderRadius: "3px",
        }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "6px" }}>
            {allTags.map((tag) => (
              <span key={tag} style={{
                ...
                padding: "2px 8px", backgroundColor: T.bg1,
                ...
                <button onClick={() => handleDeleteTag(tag)} style={{
                  background: "none", border: "none", color: T.danger,
                  cursor: "pointer", padding: "0 2px", fontSize: "12px", lineHeight: 1,
                }}>x</button>
              </span>
            ))}
          ...
          <div style={{ display: "flex", gap: "4px" }}>
            <input
              style={{
                flex: 1, padding: "4px 8px", backgroundColor: T.bg1, color: T.text,
                border: `1px solid ${T.border}`, borderRadius: "3px", fontSize: "12px",
              }}
              value={newTagInput}
              ...
              placeholder="新标签名..."
            />
            <button className="im-action-btn" onClick={handleAddTag} style={{
              padding: "4px 10px", backgroundColor: T.bg2, color: T.successDim,
              border: `1px solid ${T.border}`, borderRadius: "3px",
...
function ByTagView({ visibleTags, tagGrouped, untagged, itemTagsMap, collapsed, onToggleCollapse, onEditItem }: {
  visibleTags: string[];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      ...
      {visibleTags.map((tag) => {
        ...
        return (
          <div key={tag}>
            <button className="im-cat-btn" onClick={() => onToggleCollapse(`tag:${tag}`)}
              style={{
                width: "100%", textAlign: "left", padding: "5px 12px",
                backgroundColor: T.bg2, color: T.textSub, border: "none",
      ...
      {untagged.length > 0 && (
        <div>
          <button className="im-cat-btn" onClick={() => onToggleCollapse("tag:__untagged__")}
            style={{
              width: "100%", textAlign: "left", padding: "5px 12px",
              backgroundColor: T.bg2, color: T.textDim, border: "none",
              cursor: "pointer", fontSize: "13px", borderRadius: "3px",
              transition: "background-color 0.1s, color 0.1s",
            }}
          >
          ...
          {!(collapsed["tag:__untagged__"] ?? false) && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", padding: "6px 8px" }}>
              {untagged.map((item) => (
                <button className="im-item" key={item.id} onClick={() => onEditItem(item.id)}
                  style={{
                    padding: "4px 10px", backgroundColor: T.bg1, color: T.text,
                    border: `1px solid ${T.border}`, borderRadius: "3px",
                    cursor: "pointer", fontSize: "12px",
...
function ByItemView({ filteredItems, visibleTags, itemTagsMap, tagItemNames, collapsed, onToggleCollapse, onEditItem }: {
  ...
  const groups = useMemo(() => {
    ...
    for (const item of filteredItems) {
      if ((item.tags ?? []).length > 0) tagged.push(item); else untagged.push(item);
    }
      ...
      {groups.untagged.length > 0 && (
        <>
          <div style={{
            ...
            margin: "6px 0 2px", fontSize: "11px", color: T.textDim,
          }}>
            <span>未分类</span>
...
function ItemRow({ item, tags, onEdit, onTagHover, onTagLeave }: {
  item: ItemDefinition;
  tags: string[];
  onEdit: () => void;
          ...
          <span style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
            {tags.map((tag) => (
              <span
                ...
                onMouseEnter={(e) => { e.stopPropagation(); onTagHover(tag, e.currentTarget); }}
                onMouseLeave={(e) => { e.stopPropagation(); onTagLeave(); }}
                style={{
                  padding: "1px 7px",
