...
const SLOT_LABELS: Record<string, string> = {
  ...
  lowerBody: "下半身", lowerUnderwear: "下半身内衣", hands: "手",
  feet: "脚", shoes: "鞋子",
...
export default function ActionMenu({
  ...
  const subHeader = (title: string) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border}`, marginBottom: "8px", paddingBottom: "2px" }}>
      <span style={{ color: T.accent, fontWeight: "bold" }}>== {title} ==</span>
      <button onClick={() => setMenuMode("normal")}
        style={{ background: "none", border: `1px solid ${T.border}`, color: T.textSub, cursor: "pointer", padding: "2px 8px", borderRadius: "3px", fontSize: "12px" }}
      >[返回]</button>
    </div>
  );

  const container: React.CSSProperties = {
    fontSize: "13px", color: T.text, backgroundColor: T.bg1, padding: "12px", borderRadius: "4px",
  };
      ...
      <div style={container}>
        ...
        {moveAction.targets.map((target, idx) => (
          <button
            ...
            onClick={() => { onMove(target.targetCell, target.targetMap); setMenuMode("normal"); }}
            disabled={disabled}
            style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.actionMove, cursor: disabled ? "not-allowed" : "pointer" }}
          >
            [{idx + 1}] {target.targetMapName ? `${target.targetMapName} - ` : ""}{target.targetCellName}
      ...
      <div style={container}>
        ...
        {lookAction.targets.map((target, idx) => (
          <button
            ...
            onClick={() => { onLook(target.targetCell, target.targetMap); setMenuMode("normal"); }}
            disabled={disabled}
            style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.actionLook, cursor: disabled ? "not-allowed" : "pointer" }}
          >
            [{idx + 1}] {target.targetMapName ? `${target.targetMapName} - ` : ""}{target.targetCellName}
          </button>
        ))}
      ...
      <div style={container}>
        ...
        {outfitAction.outfitTargets.map((ot) => (
          <button
            ...
            disabled={disabled}
            style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.accent, cursor: disabled ? "not-allowed" : "pointer" }}
          >
            {ot.outfitName}
    ...
    return (
      <div style={container}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.border}`, marginBottom: "8px", paddingBottom: "2px" }}>
          ...
          <button onClick={() => setMenuMode("outfitSelect")}
            style={{ background: "none", border: `1px solid ${T.border}`, color: T.textSub, cursor: "pointer", padding: "2px 8px", borderRadius: "3px", fontSize: "12px" }}
          >[返回]</button>
        </div>
        {slotsWithItems.map(([slot, items]) => (
          <div key={slot} style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "4px" }}>
            ...
            {items.length === 1 ? (
              ...
              <select
                style={{ padding: "2px 6px", backgroundColor: T.bg3, color: T.text, border: `1px solid ${T.borderLight}`, borderRadius: "3px", fontSize: "12px", cursor: "pointer" }}
  ...
  return (
    <div style={container}>
      <div style={{ color: T.accent, borderBottom: `1px solid ${T.border}`, marginBottom: "4px", paddingBottom: "2px", fontWeight: "bold" }}>
        == 行动 ==
      </div>
      ...
      {tabs.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", borderBottom: `1px solid ${T.border}`, marginBottom: "8px" }}>
          <button style={tabStyle(ALL_TAB)} onClick={() => setActiveTab(ALL_TAB)}>[全部]</button>
          {tabs.map((tab) => (
      ...
      {showBasic && (
        <div style={{ marginBottom: "8px" }}>
          ...
          {moveAction && (
            <button onClick={() => setMenuMode("moveDetail")} disabled={disabled}
              style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.actionMove, cursor: disabled ? "not-allowed" : "pointer" }}>
              [移动...]
          ...
          {lookAction && (
            <button onClick={() => setMenuMode("lookDetail")} disabled={disabled}
              style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.actionLook, cursor: disabled ? "not-allowed" : "pointer" }}>
              [查看...]
          ...
          {outfitAction && (
            <button onClick={() => setMenuMode("outfitSelect")} disabled={disabled}
              style={{ ...btnBase, backgroundColor: disabled ? T.borderDim : T.bg2, color: disabled ? T.textDim : T.accent, cursor: disabled ? "not-allowed" : "pointer" }}>
              [换装...]
            </button>
          )}
            ...
            {catActions.map((action) => {
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  title={tooltip}
                  style={{ ...btnBase, backgroundColor: isDisabled ? T.borderDim : T.bg2, color: isDisabled ? T.textDim : T.actionConfigured, cursor: isDisabled ? "not-allowed" : "pointer" }}
                >
                  {action.name}
                  {needsNpc && <span style={{ color: T.textSub, fontSize: "11px" }}> [NPC]</span>}
                  {action.enabled === false && action.disabledReason && (
                    <span style={{ color: T.danger, fontSize: "11px", marginLeft: "6px" }}>({action.disabledReason})</span>
                  )}
                </button>
              );
            })}
