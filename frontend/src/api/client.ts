import type { GameState, GameAction, ActionResult, GameInfo, GameDefinitions, RawCharacterData, TraitDefinition, TraitGroup, ClothingDefinition, ItemDefinition, ActionDefinition, DecorPreset, RawMapData } from "../types/game";
...
export async function selectGame(gameId: string): Promise<{ success: boolean; message: string }> {
  const res = await fetch("/api/games/select", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ gameId }),
  });
  if (!res.ok) throw new Error(`Failed to select game: ${res.status}`);
  return res.json();
}
...
export function connectWebSocket(
  ...
  ws.onclose = () => {
    ...
    setTimeout(() => connectWebSocket(onStateUpdate, onGameChanged), 2000);
  };
