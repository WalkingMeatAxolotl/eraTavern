...
- 可选: seller（NPC ID）, priceKey（资源key，默认money）, amount（数量，默认1）, location（地图ID）, favChange（好感变化）, timeCost（默认5）, text（输出文本）
    ...
    "outcomes": [
      {
        ...
        "effects": ["item add stolen_goods 1", "experience theft +1"],
...
```json
{
  ...
  "costs": [
    {"type": "resource", "key": "money", "amount": 10}
  ],
  "outcomes": [
    {
      "label": "成功",
